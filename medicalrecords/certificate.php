...
require_once '../include/functions.php';
	...
	$sql = $conn->prepare("SELECT * FROM tbl_patient_info WHERE pi_id = '$id'");
	$sql->execute();
	$sql_data = $sql->fetch();

	$day_examined = date("jS", strtotime($sql_data['mh_date_examination']));
	$month_examined = date("F", strtotime($sql_data['mh_date_examination']));
	$year_examined = date("Y", strtotime($sql_data['mh_date_examination']));

	$day_exam = date("d", strtotime($sql_data['mh_date_examination']));
...
<body onload="window.print()" style="width:100%; height:99%; border:solid 0px; background-repeat: no-repeat; background-size: 100% 100%;">
	...
	<div class="main">
		...
		<div class="para1">
			...
			was seen and examined on <label id="exam_date"><?php echo $month_examined; ?> <?php echo $day_exam; ?> <?php echo $year_examined; ?></label> and was diagnosed to have 
			<label id="diagnosis"><?php echo $sql_data['mh_symp_diag']; ?></label>.</p>
		</div>

		<div class="para2">
			<p class="a2">Recommendation: <label id="treatment"><?php echo $sql_data['mh_treat']; ?></label></p>
		</div>
		...
		<div class="phys">
			...
			<p><label id="physician"><?php echo $sql_data['mh_physician']; ?></label></p>
			<p>MEDICAL CONSULTANT/CLINIC PHYSICIAN</p>
			<p>License No. <label id="lic_no"><?php echo $sql_data['mh_license']; ?></label></p>
		</div>
	</div>
