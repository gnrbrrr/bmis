...
$sql = $conn->prepare("SELECT * FROM tbl_resolution WHERE is_deleted != '1'");
$sql->execute();
	...
	<div class="row">
		<div class="col-sm-12">
			<div class="white-box">
				...
				<p class="text-muted m-b-30">Display list of Resolution</p></p>
						...
						<tbody>
							...
							<?php
								if($sql->rowCount() > 0)
								{
									...
									{
										$dateapp = date("M d, Y", strtotime($sql_data['re_date']));
							?>
										<tr>
											<td><?php echo utf8_encode($sql_data['re_itemno']); ?></td>
											<td style="text-align:center;"><a href="<?php echo WEB_ROOT; ?>resolution/resolution_form.php?id=<?php echo $sql_data['re_id']; ?>"><i class="fa fa-print fa-fw"></i></a></td>
											<td><?php echo $sql_data['re_resno']; ?> </td>
											<td><div class="title-column"><?php echo $sql_data['re_title']; ?></div></td>
											<td><?php echo $dateapp; ?></td>
											<td><?php echo $sql_data['re_committee']; ?></td>
											<td><?php echo $sql_data['re_remarks']; ?></td>
											<td>
