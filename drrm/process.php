...
function add_data()
{
	...
	$chk = $conn->prepare("SELECT * FROM tbl_inventory_drrm WHERE in_itemdesc = '$itemDesc' AND in_serialno = '$serialNo' AND is_deleted != '1'");
	$chk->execute();
	if($chk->rowCount() > 0)
	{
        ...
        $sql = $conn->prepare("INSERT INTO tbl_inventory_drrm (in_item, in_itemdesc, in_serialno, in_amt, in_dop, in_condition, in_qty, is_deleted) 
		...
		$up = $conn->prepare("UPDATE tbl_inventory_drrm SET uid = '$uid' WHERE in_id = '$id'");
		$up->execute();
...
function modify_data()
{
	...
	$chk = $conn->prepare("SELECT * FROM tbl_inventory_drrm WHERE in_itemdesc = '$itemDesc' AND in_serialno = '$serialNo' AND uid != '$id' AND is_deleted != '1'");
	$chk->execute();
	if($chk->rowCount() > 0)
	{
		...
		$sql = $conn->prepare("UPDATE tbl_inventory_drrm SET in_item = '$item', in_itemdesc = '$itemDesc', in_serialno = '$serialNo', in_amt = '$amt', in_dop = '$dop', in_condition = '$condition', in_qty = '$qty' WHERE uid = '$id'");
		$sql->execute();
...
function delete_data()
{
	...
	$sel = $conn->prepare("SELECT * FROM tbl_inventory_drrm WHERE uid = '$id'");
	$sel->execute();
	$sel_data = $sel->fetch();
	...
	$sql = $conn->prepare("UPDATE tbl_inventory_drrm SET is_deleted = '1' WHERE uid = '$id'");
	$sql->execute();
