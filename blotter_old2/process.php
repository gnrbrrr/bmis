...
function add_data()
{
	...
	if($chk->rowCount() > 0)
	{
		...
		$sql = $conn->prepare("INSERT INTO tbl_blotter  (status, blotter_no, complainant, victim, suspect_firstname, suspect_lastname, suspect_middlename, witness1, witness2, natureofcase, timeofoccurence, dateofoccurence, placeofoccurence, narrative, other_nature_case, date_filed, articles, disposition, is_deleted) 
													VALUES ('$status', '$blotterno', '$complainant', '$victim', '$fsuspect', '$lsuspect', '$msuspect', '$witness1', '$witness2', '$incident', '$timeoccurence_formatted', '$dateofoccurence', '$placeofoccurence', '$narrative', '$otherf', '$datefile', '$articles', '$disposition', '0')");
		$sql->execute();
...
function modify_data()
{
	...
	if($chk->rowCount() > 0)
	{
		...
		$sql = $conn->prepare("UPDATE tbl_blotter SET status = '$status', blotter_no = '$blotterno', complainant = '$complainant', victim = '$victim', suspect_firstname = '$fsuspect', suspect_lastname = '$lsuspect', suspect_middlename = '$msuspect', witness1 = '$witness1', witness2 = '$witness2', 
