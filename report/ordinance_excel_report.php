...
$output = '';
        ...
        if($date_from === "" && $date_to === ""){
            ...
            $query = "SELECT * FROM tbl_ordinance WHERE (ord_date BETWEEN '$date_from' and '$date_to') AND is_deleted != '1' ORDER BY ord_date";
            $result = mysqli_query($connect, $query);
            if(mysqli_num_rows($result) > 0)
            {
                //title of The excel file
                echo '<center>Barangay Ordinance Record</center>';
                ...
                <table class="table" bordered="1">  
                                    <tr>  
                                        ...
                                        <td class="tdlabel">Ordinance No.</td>
                ...
                {
                    $date = date("M d Y", strtotime($row["ord_date"]));
                    $output .= '
                                        <tr>  
                                            <td>' . $row["ord_itemno"].'</td>
                                            <td>' . $row["ord_no"].'</td>
                                            <td>' . $row["ord_title"].'</td>
                                            <td>' . $date.'</td>
                                            <td>' . $row["ord_committee"].'</td>
                                        </tr>
                    ';
                }
                    $output .= '</table>';
                    header('Content-Type: application/xls');
                    header('Content-Disposition: attachment; filename=Barangay Ordinance Report List.xls');
                    echo $output;
