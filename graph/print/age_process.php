...
<?php
		...
		// Infant
			$sql1 = $conn->prepare("SELECT *, COUNT(age) as total_infant FROM tbl_resident WHERE is_deleted != '1' AND age = '0'");
			$sql1->execute();
			if($sql1->rowCount() > 0)
			{
		...
		// Children
			$sql2 = $conn->prepare("SELECT *, COUNT(age) as total_children FROM tbl_resident WHERE is_deleted != '1' AND age BETWEEN '1' and '6'");
			$sql2->execute();
			if($sql2->rowCount() > 0)
			{
		...
		// Youth
			$sql3 = $conn->prepare("SELECT *, COUNT(age) as total_youth FROM tbl_resident WHERE is_deleted != '1' AND age BETWEEN '7' and '17'");
			$sql3->execute();
			if($sql3->rowCount() > 0)
			{
		...
		// Adult
			$sql4 = $conn->prepare("SELECT *, COUNT(age) as total_adult FROM tbl_resident WHERE is_deleted != '1' AND age BETWEEN '18' and '59'");
			$sql4->execute();
			if($sql4->rowCount() > 0)
			{
		...
		// Senior
			$sql5 = $conn->prepare("SELECT *, COUNT(age) as total_senior FROM tbl_resident WHERE is_deleted != '1' AND age >= '60'");
			$sql5->execute();
			if($sql5->rowCount() > 0)
			{
